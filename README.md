...
2024-7-23 to 2024-7-24

目前对于文字的识别已经十分准确, 但是对于图标的识别还是十分欠缺的, 只有在图标中有文字时才可以被识别出来, 所以我们需要改善对于图标的识别

图标可以分为线条图标和颜色填充图标.



一开始我希望找到现有的模型或api来实现此功能

1. google cloud

配置完成之后, 在测试时, 出现了网络连接错误, 而后经过检查, 可能是服务没有进行付费的问题 (实际上显示服务已经开启), 也有可能是使用vpn的原因 (之前在调用腾讯ocr时, 由于vpn的缘故)

2. Tencent cloud

配置完成之后, 进行了测试, 但是从测试结果我们可以看出tencent cloud api返回的信息中不包含位置信息, 所以此api没有办法用来进行图标的标记

![](./data/doc/icon_identification_test2/result1.png)



所以, 我以YOLO模型为基础对识别图标模型进行了训练, 目前还在训练中. 该训练项目在其他的项目中, 训练成熟后会应用到现在项目中.
